...
void ReplayDispatchHandler::handle(Hypertable::EventPtr &event_ptr) {
  ...
  if (event_ptr->type == Event::MESSAGE) {
    ...
    location_error = true;
    HT_ERROR_OUT << "replay failure for at " << event_ptr->proxy << "-" << e << HT_END;
  }

  {
    ...
    if (location_error)
      m_location_errors[event_ptr->proxy] = event->type;
    else if (range_error)
...
void ReplayDispatchHandler::get_completed_ranges(set<QualifiedRangeSpec> &ranges) {
  HT_ASSERT(m_outstanding==0);
  foreach(QualifiedRangeSpec &range, m_completed_ranges)
    ranges.insert(range);
}
