...
namespace Hypertable {
  ...
  public:
    ...
    void player_complete(int64_t op_id, int type, const String &location,
                         uint32_t attempt, const map<uint32_t, int> &error_map, Timer *timer=0);

    void phantom_prepare_complete(int64_t op_id, uint32_t attempt,
        const map<QualifiedRangeSpec, int> &error_map, Timer *timer=0);

    void phantom_commit_complete(int64_t op_id, uint32_t attempt,
        const map<QualifiedRangeSpec, int> &error_map, Timer *timer=0);
